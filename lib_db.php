...
function connect(){
	global $link;
	if ($link) return 0;
	$link = mysqli_connect('localhost', 'duongnam', 'tamphuong');
	if (!$link) {
	    die('<br/>Khong ket noi duoc: ' . mysqli_error($link));
	}	
	mysqli_select_db($link,'cnw_ngocboi') or die('Could not select database.');
...
function exec_update($sql){
	...
	//kiem tra
	if ($err){
		print("Khong thể update/create duoc,ERROR=[" . $err . "]" );
		print(  "COUNT=[0]" );
		return -1;
	}
